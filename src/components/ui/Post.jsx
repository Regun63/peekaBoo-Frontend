...
const Post = ({ post }) => {
  ...
  const handleFollower = async () => {
    try {
      ...
      const res = await axios.post(
        `https://peekaboo-backend-1.onrender.com/api/peekaBoo/user/follow_or_unfollow/${post?.author?._id}`,
        {},
  ...
  const handleLike = async () => {
    try {
      ...
      const res = await axios.get(
        `https://peekaboo-backend-1.onrender.com/api/peekaBoo/post/${post?._id}/${action}`,
        { withCredentials: true }
      );
      if (res.data.success) {
  ...
  const commentHandler = async () => {
    ...
    try {
      const res = await axios.post(
        `https://peekaboo-backend-1.onrender.com/api/peekaBoo/post/${post?._id}/comment`,
        { input },
        {
          headers: {
  ...
  const handleBookmark =async () => {
    ...
    try {
      const res = await axios.get(
        `https://peekaboo-backend-1.onrender.com/api/peekaBoo/post/${post?._id}/bookmark`,
